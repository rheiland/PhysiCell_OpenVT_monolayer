...
mech_voxel_size = float(sys.argv[3])
print("mech_voxel_size=",mech_voxel_size)
...
def plot_nbrs():
    ...
    try:
        ...
        mcds = pyMCDS(xml_file, microenv=False, graph=True, verbose=True)
    except:
    ...
    # circles(xv,yv, s=r, color=ctype)
        ...
        circles(xv,yv, s=r, color='lightgray')
        circles(xv,yv, s=0.1, color='black')

        # show IDs at cell centers
        # for id in nbrs.keys():
        offset=0.2
        for id in range(num_cells):
            # ax.text(xv[id], yv[id], ids[id], fontsize=10)
            ax.text(xv[id]+0.4, yv[id]-0.4, f'{int(ids[id])} x={xv[id]:.3f}', fontsize=10)
    except:
        print("invalid frame")

    y_values = [-mech_voxel_size, 0, mech_voxel_size]
    xmin_vals = [-mech_voxel_size, -mech_voxel_size, -mech_voxel_size]
    xmax_vals = [mech_voxel_size ,mech_voxel_size, mech_voxel_size]
    colors = ['red', 'red']
    linestyles = ['--', '-']
    plt.hlines(y=y_values, xmin=xmin_vals, xmax=xmax_vals, colors='r', linestyles='--')

    x_values = [-mech_voxel_size, 0, mech_voxel_size]
    ymin_vals = [-mech_voxel_size, -mech_voxel_size,-mech_voxel_size]
    ymax_vals = [mech_voxel_size, mech_voxel_size, mech_voxel_size]
    colors = ['red', 'red']
    linestyles = ['--', '-']
    plt.vlines(x=x_values, ymin=ymin_vals, ymax=ymax_vals, colors='r', linestyles='--')
